            ...
            for resolution in resolutions:
                ...
                for i in range(6 * resolution):
                    ...
                    if segment.size > 0:
                        ...
                        segment_trend = np.polyfit(range(min(3, segment.shape[1])), 
                                                 np.mean(segment[:, -min(3, segment.shape[1]):], 1)[0] if segment.shape[1] > 1 else 0
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=port, debug=debug)from __future__ import annotations
